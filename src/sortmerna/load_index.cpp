...
void load_index_stats(
	// IN <- opts
	vector< pair<string, string> >& myfiles, // opts.indexfiles
	char** argv,
	int argc,
	bool yes_SQ, // opts
	char* acceptedstrings_sam, // Output
	long match, // opts
	long mismatch, // opts
	long gap_open, // opts
	long gap_extension, // opts
	vector<vector<uint32_t> >& skiplengths, // opts
	// OUT -> Index
...
)
{
	...
	for (uint16_t index_num = 0; index_num < (uint16_t)myfiles.size(); index_num++)
	{
		...
		{
			fprintf(stderr, "\n  %sERROR%s: The index '%s' does not exist.\n", startColor, 
				"\033[0m", (char*)(myfiles[index_num].second + ".stats").c_str());
			fprintf(stderr, "  Make sure you have constructed your index using the command `indexdb'. See `indexdb -h' for help.\n\n");
			exit(EXIT_FAILURE);
		}
