...
[
  ...
  [
    if test "$withval" = "yes" ; then
      ...
      AC_MSG_WARN([cannot support reading compressed files without zlib])
    else
  ...
  if test "$zlib_found" = "no"; then
    AC_MSG_ERROR([cannot support reading compressed files without zlib])
  fi

  if test "$zlib_found" = "yes"; then
    HAVE_ZLIB_INCLUDES="-I$zlib_prefix/include"
    LDFLAGS="$LDFLAGS `HAVE_REMOVE_STANDARD_LIB_DIRS(-L$zlib_prefix/lib)`"
  fi

  HAVE_ZLIB_LIBS="-lz"

  AC_SUBST(HAVE_ZLIB_INCLUDES)
  AC_SUBST(HAVE_ZLIB_LIBS)
])